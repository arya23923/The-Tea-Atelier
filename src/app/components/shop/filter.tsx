...
const Filter:FC<prop> = ({isOpen, onClose, onApply}) => {
    ...
    return(
       <div className={`fixed inset-y-0 left-0 bottom-0 mt-40 transition-all duration-300 z-10 w-90 md:w-170 md:mt-48 text-black bg-amber-100 p-5 ${montserrat.className} ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
            <div onClick={onClose}><Image src={cross} alt="cross" className="w-10 h-auto justify-self-end"/></div>
            <div className="p-5 justify-center">
                <p className="p-5 text-xl md:text-lg">Price</p>
                <RangeSlider
                ...
                <div className="flex flex-col p-5 pb-15 pt-10 md:flex-row">
                    <label className="text-lg">₹ <input type="number" value={min} onChange={(e) => setMin(Number(e.target.value))} className="border border-amber-900 w-50 pr-5 pl-5 pt-3 pb-3 text-lg"/></label>
